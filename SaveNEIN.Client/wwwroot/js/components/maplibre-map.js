...
window.MapLibreImpactMap = (function ()
{
    ...
    // Radius mode keeps public previews fully client-side after cached population data loads.
    ...
    async function loadCompetitors()
    {
        try
        {
            ...
            const res = await fetch('/api/casino-competitors');
    ...
    async function loadCountyContext(fips, lite = false, loadingText = "Loading Data...", manageLoading = true, isPrefetch = false)
    {
        ...
        const promise = (async () =>
        {
            ...
            {
                const ts = Date.now();
                let res = await fetch(`/api/Impact/county-context/${fips}?lite=${lite}&_ts=${ts}`, {
                    signal: controller.signal,
                    cache: 'no-store'
                });
                if (!res.ok)
                {
                    res = await fetch(`/api/census/population-heatmap/${String(fips).substring(0, 2)}`, {
                        signal: controller.signal,
                        cache: 'no-store'
                    });
                }
    ...
    return {
        ...
        configureZonesFromInputs: () =>
        {
            const read = id => Number(document.getElementById(id)?.value);
            const values = [read('zone-tier-1-miles'), read('zone-tier-2-miles'), read('zone-tier-3-miles')];
            const count = Math.max(1, Math.min(3, Math.round(read('zone-tier-count') || 3)));
            if (values.every(value => Number.isFinite(value) && value > 0) && values[0] < values[1] && values[1] < values[2])
            {
                zoneCount = count;
                CIRCLE_RADII.tier1 = values[0] * MILE_TO_METERS;
                CIRCLE_RADII.tier2 = values[1] * MILE_TO_METERS;
                CIRCLE_RADII.tier3 = values[2] * MILE_TO_METERS;
                if (markerPosition)
                {
                    updateCircles(markerPosition);
                    calculateImpact();
        ...
        getScenarioSnapshot: () =>
        {
            ...
            return {
                version: 'economic-impact-client-v1',
                stateFips: currentStateFips,
                countyFips: currentCountyFips,
                markerLatitude: markerPosition?.lat ?? null,
                markerLongitude: markerPosition?.lng ?? null,
                zoneConfiguration: {
                    count: 3,
                    tiers: [15, 30, 60],
                    unit: 'minutes-drive-time',
                    mode: riskZoneMode
                },
                previewOutputs: {
                    tier1Adults: t1PopRegional,
                    tier2Adults: t2PopRegional,
                    tier3Adults: t3PopRegional,
                    countyBreakdown: byCounty
                }
            };
        },
        getMap: () => map,
