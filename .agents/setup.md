...
2. Ensure the database and Valhalla containers are running (`docker compose up -d savenein-db valhalla`).
...
> **Note:** Frontend asset building (`npm install`, `copy-libs`, `build:css`) is handled automatically by MSBuild pre-build targets in `SaveNEIN.Client.csproj`. No separate build step is needed.
...
- Forbidden behavior:
  ...
  - do not stop any `SaveNEIN.Server`, `dotnet`, `npm`, or `node` process unless it is the exact pid recorded by `scripts/dev/.local-dev-server.pid`
...
- Do not run `dotnet build SaveNEIN.Server/SaveNEIN.Server.csproj` in parallel with an active watcher unless the watcher is first stopped through the repo-managed scripts.

## Database Seeding
The backend (`SaveNEIN.Server`) is configured to automatically apply EF Core migrations and seed TIGER (Census) data on startup via a fire-and-forget background task. It also pre-warms the MVT tile cache for the initial map view.
