...
case "$validation_db" in
  savenein_calibration_validation_[a-z0-9]*) ;;
  *) echo "Unsafe validation database name" >&2; exit 64 ;;
esac
case "$validation_dir" in
  /tmp/savenein-calibration-validation.*) ;;
...
"${validation_compose[@]}" run --rm --no-deps \
  ...
  --validate-incumbent-calibration \
  "$validation_db" \
  http://valhalla:8002

echo "Remote live incumbent-backtest calibration validation passed."
