...
> - **SUCCESS:** `SaveNEIN.Server.Tests` passes 174/174 tests in an exact-index detached worktree for the geographic-residual-pattern tranche, including stored-run report integration, dynamic origin reconciliation, artifact immutability/cache identity, draft-run rejection, model-foundation seed metadata/default upgrades, distinct casino/racino rule seeding and obsolete-fixture removal, Indiana conditional base/supplemental gaming-tax schedules and component distributions, schema defaults, multi-provider jurisdiction composition, Indiana annual facility-profile/employment parsing, Michigan/Ohio/Pokagon source discovery and evidence validation, candidate/incumbent route cache identity/reuse, exact incumbent-to-held-out-scenario coordinate reuse, radial ZCTA study-region selection, official-missing-value handling, market-scoped IRS reconciliation, INDOT stable-identity handling, annual inventory composition, Census CBP local-inventory/labor resolution, observed unit-day capacity benchmarking, regulator-observed jobs/GGR benchmarking, explicit population projection/backcast, validated jurisdiction problem-gambling prevalence resolution, immutable validation residual aggregation by market/jurisdiction/holdout group, pair-specific Valhalla matrix eligibility/unroutable isolation, and the numerical-robustness cases below. The same exact-index validation-harness tree builds with 0 warnings and 0 errors.
...
> - **SUCCESS:** Indiana incumbent supplemental wagering tax now uses effective-dated, venue-specific evidence instead of one countywide approximation. Eight IC 4-33-12-1.5 statutory-quotient rules preserve each property's official FY2017 admissions-tax numerator, taxable-AGR denominator, four-decimal rate, and 3.5% cap; current IGC evidence supplies Hard Rock's 2.98%, Vigo County remains the statutory 2.9%, and explicit zero rules document the historic-hotel-district exclusion and the expired racino supplemental fee. Stable venue identity prevents the two Lake County properties from inheriting each other's rate, malformed sourced arithmetic fails closed, and incumbent gaming-tax-loss accounting now includes validated supplemental tax. The live PostgreSQL run also caught and corrected a JSONB/string cleanup-query incompatibility that the in-memory provider could not expose. The isolated fiscal exact-index tree passes 179/179 server tests and builds the validation harness with 0 warnings and 0 errors; the combined worktree, including the still-running calibration tranche, passes 181/181. Checksum-pinned artifact `70302d468e5c5622c4ea47c9293297aa9fcb31b487c87e79b6e2151e150df590` applied every model migration in fresh disposable VPS PostGIS database `savenein_fiscal_validation_20260813_v9`, using 92 Indiana county and 974 place geometries. It seeded 13 validated supplemental rules (eight sourced quotients and two explicit zero rules), retained zero obsolete rules, and reproduced $402,299.60 Ameristar, $341,153.82 Blue Chip, $1,064,156.66 Hard Rock, and $350,687.05 Terre Haute liabilities from the published taxable-AGR examples; French Lick and Hoosier Park returned zero. All failed/successful disposable databases and remote artifacts were removed. Recipient-level statewide set-asides outside the northeast host allocation and broader jurisdiction coverage remain open, so production fiscal acceptance stays unchecked.
> - **SUCCESS:** Gaming-tax distribution rules now retain named statutory recipients, facility eligibility, per-period/annual caps, ordered recipient subtraction, one substantive residual recipient for penny-exact reconciliation, and municipality/county/region/state rollups. The same named allocations are persisted in each run's rule-provenance JSON rather than disappearing into aggregate host shares. Effective-dated IC 4-33-12-8.5 data allocate Terre Haute's $350,687.05 supplemental liability to the City of Terre Haute ($140,274.82), Vigo County ($105,206.12), the Vigo County school corporation ($52,603.06), and West Central 2025 ($52,603.05); IC 4-33-12-8.7 retains Fort Wayne/Allen County/Northeast Indiana RDA allocations of $1.26M/$1.26M/$280k. The isolated exact-index fiscal tree passes 181/181 and builds the harness with 0 warnings/errors; the combined worktree, including calibration WIP, passes 183/183. Checksum-pinned artifact `3fe7ea7dd9e40806c337e16ebb4e0c4ac8a7c59d18484d730b4f154ecc44d10d` reproduced all amounts and five named distribution rules in fresh VPS PostGIS database `savenein_fiscal_validation_20260813_v12` using 92 Indiana counties and 974 Indiana place geometries (565 active), then returned `CLEANUP_OK` after removing the database, container, script, and artifact. Generic commercial-casino and Lake County distributions remain fail-closed because IC 4-33-12-9(k)'s base-year/cap evidence has not yet been encoded; the current racino county fee and other statewide set-asides also remain open.
> - **SUCCESS:** Official 2023 Census County Business Patterns state/county archives now supply the production `local-economic-inventory` provider instead of disposable fixtures. The adapter maps only NAICS 72 restaurants/hospitality, 44-45 retail, and 71 arts/entertainment/recreation into displacement sectors; preserves establishments, March employment, annual payroll, noise flags, raw-source hash, and transform checksum; and leaves receipts/sales null because CBP does not publish them. Geography integrity is fail-closed before network access: a state file may populate only a host-state scope and a county file only its host-county scope, so broad source data cannot be mislabeled as municipality, metro, or custom-area evidence. Checksum-pinned remote probe artifact `3651a55180df93d56316bb21970ae7fc38c471ae73e9a67722dbaa499cf8b88c` reproduced Indiana state transform checksum `e780cad8a59394b52328cd0ee4e6fc68fb3bf8c382bf4c4958c8464814601edd` and Allen County checksum `3e81c63908233dde891378875ac7d671e79d7b886f44580f4e6faf47dccdd610`. Indiana state rows contain 291,843 hospitality, 335,879 retail, and 36,609 arts/recreation employees; Allen County contains 18,505, 22,348, and 2,581 respectively. State casino-gambling payroll/employment resolve a $43,264.02 direct/incumbent annual wage and all-industries data resolve a $57,497.32 indirect wage. Allen County has no published casino row, so direct/incumbent wages remain explicit versioned fallbacks while its all-industries payroll resolves a $56,031.45 indirect wage; state wages are not mislabeled as county-specific. Migration `022_employment_assumption_provenance.sql` persisted all three resolved wages plus their source basis and passed on disposable VPS schema `savenein_employment_validation_20260813_v1` with four columns and a validated nonnegative constraint; the database was removed after success. Report HTML/PDF/CSV disclose the stored wages and provenance. Occupation mix, construction job-years, and regional indirect/induced multipliers still require production evidence, so Phase E employment remains open.
...
> - **SUCCESS:** Stored-run origin presentation now uses one backend `origin-summaries` contract over the immutable origin results rather than UI-only truncation. It groups by native origin/ZCTA, county/parish, state/territory, MSA, CSA, country, candidate-containing host region, persisted scenario jurisdiction, and in-state/out-of-state/international relationship; candidate host codes come from exact stored origin polygons and in-jurisdiction membership comes from the run's persisted local-origin set. Configurable top-N and minimum-share thresholds produce one `Other origins` residual that exactly reconciles resident demand, induced demand, proposed GGR, jurisdiction captures, and outside-option capture to all underlying rows. The web workbench uses the service with a 25-origin/0.1% display threshold, while detailed paging and CSV retain every origin. HTML/PDF reports now append an explicit reconciled residual beyond their configured top-N, and the server refuses to synthesize a ZIP/ZCTA summary from non-ZCTA computational origins without a versioned crosswalk. Focused service/controller/report tests pass 9/9 as part of the 163/163 Release suite.
> - **SUCCESS:** IGC FY2025 facility profiles now supply regulator-reported total employment for every Indiana commercial casino/racino instead of leaving direct job density at zero. The transform stores 10,112 employees across 13 facilities and joins them to the same sealed 2025 observed-performance rows used by the model. Snapshot checksum `38fffb450f83fb10496b77154d1d1596cc2b600a6f3a3f3103c83d9fff846852` binds the unchanged raw source hash to transform `igc-facilities-units-annual-attributes-employment-v4`, so the employment-bearing snapshot cannot collide with the earlier facility transform. Checksum-pinned harness artifact `a8360d158490ad456bd62a5833a508abc36a740038ff7f523b2245178a8f15aa` ingested sealed competitor snapshot `8bdd91b1-0419-4a4f-a5e5-e41fea9ea9af` and performance snapshot `1c94bdc3-3197-4dd8-baab-2f0d15452cb4` into disposable VPS PostGIS database `savenein_employment_validation_20260813_v2`: 468 rows reconcile to $2,436,931,996.47 comparable GGR. The revenue-weighted benchmark is 4.14948 jobs per $1M GGR across all 13 facilities, with a disclosed 2.48916–22.30277 property range. Runtime direct and incumbent job-loss calculations use it automatically only when the corresponding versioned parameter remains zero; a positive configured value explicitly supersedes it, and both the applied value and observed benchmark are persisted in employment provenance. Migration `024_reported_casino_employment.sql` passed with one reported-employment column and one validated positive-value constraint; the disposable database was dropped. The IGC source does not define occupation mix or FTE status, and construction/indirect multipliers remain zero-safe, so Phase E remains open.
> - **SUCCESS:** Scenario-year eligible population no longer silently reuses the source observation year. `demographics.population_annual_growth_rate` is a bounded, versioned, advanced/user-overridable parameter with units, an explicit constant-population default, and instructions to retain the jurisdiction-appropriate official projection source in parameter-set provenance. `PopulationProjectionCalculator` applies the same compound annual formula to every origin after legal-gaming-age calculation, supports an explicit backcast, rejects unsafe rates and horizons, and preserves the selected age snapshot/year plus scenario effective year and resolved parameter in the immutable run. Five focused cases cover projection, no-growth identity, backcast, and bounds; its exact staged tree passed 166/166 tests. Geography-specific population-control reconciliation remains open.
> - **SUCCESS:** Indiana problem-gambling prevalence now resolves from effective-dated validated jurisdiction evidence instead of a silent universal value. The official Indiana FSSA/DMHA 2025 State Epidemiological Outcomes Workgroup report was downloaded with SHA-256 `9414096e164ce4a68ba700a46e659e662328403aaa82ec0209c0d03a25a47ee3`; text extraction and visual inspection of page 117 confirmed Table 8.2's 2021 DSM-V gambling-disorder estimate of 4.1% (95% CI 1.8%-9.0%) for Indiana adults. Runtime priority is explicit user/scenario/jurisdiction parameter value, then validated local evidence, then a labeled national/system fallback; therefore a national prior cannot hide available local evidence and an explicit zero override remains authoritative. Each stored social-cost domain persists the applied prevalence, source layer, rule ID, source URL/checksum, observation year, instrument, population, confidence interval, exposure response, and population/scenario years. Checksum-pinned harness artifact `3895994d33fc7eb1457be1d3872a60ab641e26f597b7beda59f941699bedecae` reproduced the one validated effective rule in disposable VPS PostGIS database `savenein_social_validation_20260813_v1`, then dropped the database and removed remote artifacts. Its exact staged tree passes 170/170 tests and builds the validation harness with 0 warnings and 0 errors. The prevalence is observed existing prevalence—not a causal estimate of incremental cases—so exposure response and nonoverlapping cost domains remain zero-safe and Phase E social acceptance stays open.
> - **SUCCESS:** Validation evaluations now measure and persist geographic residual patterns for both gravity and the independent comparable model. Each training/holdout/benchmark partition is grouped independently by market, jurisdiction, and documented holdout group; stored rows preserve observed/predicted totals, signed and mean residuals, MAE/MAPE/SMAPE, and over/under/exact counts with reconciliation constraints and finalized-evaluation immutability. The evaluation API exposes those first-class rows instead of requiring consumers to infer geography from a flat result list. Migration `025_validation_geographic_residual_patterns.sql` and the complete migration validator were tested through checksum-pinned archive `a8359ddef92d774774c9bd64c5723a7c10623a81c8f6226cf3b64588bb463211`: the VPS applied migrations 005-025 twice to disposable PostGIS database `savenein_migration_validation_georesidual20260813v5`, passed the updated smoke transaction, found 36 immutability triggers, and removed the database and all remote artifacts. The validator exercise also repaired its stale smoke-test path and post-020 route/post-021 fiscal fixtures. The exact staged tree passes 174/174 tests and builds the validation harness with 0 warnings and 0 errors. This completes geographic residual measurement only; the active calibration grid and production-quality acceptance gates remain open.
...
> - **IMPLEMENTED / VALIDATION OUTPUT STILL PENDING (2026-08-19):** `DemandSpecificationReconciliationService` now evaluates AGI-share and eligible-adult-per-capita demand side by side from the same origin key, reconciles total demand plus state and distance-band totals, ranks the largest origin-level differences, can rank the two specifications from paired holdout MAE/MAPE/SMAPE/RMSE, and permits only explicitly validated, versioned convex ensembles whose nonnegative weights sum to 1.0. Seven focused unit tests were added for grouped reconciliation, objective-based selection, convex-ensemble behavior, unvalidated/invalid weights, mismatched origin keys, and incomplete holdout inputs. No test/CI execution result is claimed for this tranche because the current execution environment does not expose the repository's .NET toolchain and GitHub had not attached a workflow run to the commit at verification time. Empirical paired validation and production base-specification designation therefore remain open.
...
- [x] Admission or device taxes where applicable. *(Indiana's former admissions tax was repealed effective July 1, 2018; its FY2017 property-level amounts are retained only as the enacted numerator for current supplemental wagering-tax quotients. The former 1% racino supplemental fee expired before July 1, 2012 and is represented by an effective-dated zero rule. No current admission/device tax is invented.)*
...
- [ ] Implement Indiana first. *(Legal gaming age, commercial-casino/racino base schedules, historical incumbent supplemental quotients, current fixed supplemental rates, explicit non-applicability rules, and exact named northeast/Vigo recipient distributions are validated and remotely verified. Generic and Lake County distributions subject to IC 4-33-12-9(k) base-year/cap evidence, the current racino county fee, other statewide set-asides, and remaining production fiscal rules are incomplete.)*
...
- [x] Support projection to scenario year using explicit population-growth assumptions and data sources. *(The selected Census/ACS age snapshot and observation year are projected or backcast to the scenario effective year with a bounded versioned annual-growth parameter. Zero is an explicit constant-population default; nonzero official projection sources belong in parameter-set provenance and remain fully override-audited.)*
...
- [ ] Produce both demand specifications for validation runs. *(The validation-only reconciliation service now calculates paired AGI-share and eligible-adult-per-capita demand for the same origin universe, but stored validation orchestration has not yet persisted an empirical paired validation run.)*
- [x] Compare total demand, state totals, distance bands, and largest origin differences. *(`DemandSpecificationReconciliationService` performs all four comparisons and the focused test tranche covers grouped totals and origin-difference ranking.)*
- [ ] Select base specification based on validation performance. *(The service can rank paired holdout results by MAE, MAPE, SMAPE, or RMSE, but no empirical paired validation has yet designated the production base specification.)*
- [x] Support optional validated ensemble with versioned weights. *(The reconciliation contract requires a non-empty ensemble version, an explicit validated state, finite nonnegative weights, and weights that sum to 1.0.)*
- [x] Never add two alternative demand specifications together as if they represent separate demand pools. *(Only a convex combination is permitted; additive `1.0 + 1.0` weights are rejected, and a focused test verifies the ensemble total is not the sum of both demand pools.)*
...
- [x] Measure MAE, MAPE/SMAPE, RMSE, bias, rank correlation, and geographic residual patterns. *(Finalized evaluations persist all scalar metrics plus partitioned market, jurisdiction, and holdout-group residual aggregates for both gravity and comparable predictions.)*
...
- [ ] Use wage/occupation assumptions tied to relevant geography where available. *(Geography-matched CBP payroll-per-employee wages and regulator-observed Indiana direct/incumbent jobs per GGR are implemented and persisted when available; occupation mix, construction job-years, and regional indirect/induced multipliers remain open.)*
...
- [x] Keep exposure population location-sensitive.
- [x] Make social-cost geography configurable nationally.
- [x] Use national research defaults only when local/regional evidence is unavailable and label them clearly. *(Validated effective jurisdiction prevalence supersedes national/system fallback; explicit user, scenario, or jurisdiction parameter values remain authoritative and audited.)*
- [x] Support user override of major social-cost parameters.
- [x] Preserve parameter sources and ranges.
- [ ] Potential modeled domains include, subject to evidence quality:
  - [x] problem/disordered gambling prevalence;
...
- [ ] Employment. *(The tested bridge now consumes and discloses geography-matched CBP wages plus regulator-observed Indiana direct/incumbent jobs per GGR when available; occupation mix, construction job-years, and regional indirect/induced multipliers remain incomplete.)*
- [ ] Fiscal impact. *(The tested engine, Indiana base schedules, venue-specific incumbent/fixed supplemental rates, current admission/device non-applicability, and named northeast/Vigo recipient distributions are validated. Generic/Lake distributions subject to IC 4-33-12-9(k), the current racino county fee, remaining statewide set-asides, other state/local fiscal inputs, and broader jurisdiction coverage remain incomplete.)*
