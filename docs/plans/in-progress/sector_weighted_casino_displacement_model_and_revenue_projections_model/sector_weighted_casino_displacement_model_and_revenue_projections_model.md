...
> - **SUCCESS:** Migration `020_coordinate_versioned_incumbent_travel_cache.sql` prevents a stable venue ID from reusing travel after its coordinates move between immutable snapshots. Every persisted route now carries exact facility latitude/longitude and a coordinate identity; unchanged legacy rows may be reused only after bit-exact coordinate verification, while changed coordinates create a separate route. Checksum-pinned VPS artifact `134d4232c348ef1ba71f9a66b24a3a42f6c7c96aeafd0e4bbe85095b3a39eaa5` applied the migration cleanly and backfilled all 27,318 existing route rows. The three corrected 330/344/356-origin benchmark runs then completed in 34.0/15.1/13.3 seconds, reusing unchanged exact routes and resolving the newly added South Bend pairs.
...
- [ ] Implement Indiana first. *(Legal gaming age and the effective state gaming-tax schedules for commercial casinos and racinos are validated and remotely verified. Supplemental wagering tax, statewide set-asides/distributions, local caps, and other production fiscal rules remain incomplete.)*
...
- [ ] Fiscal impact. *(The tested engine and Indiana state gaming-tax schedules are validated; supplemental wagering tax, statewide/local distribution and caps, displaced-sector fiscal inputs, and broader jurisdiction coverage remain incomplete.)*
