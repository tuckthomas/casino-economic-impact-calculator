...
- [ ] Implement Indiana first. *(Legal gaming age, commercial-casino/racino base schedules, historical incumbent supplemental quotients, current fixed supplemental rates, explicit non-applicability rules, and the enacted northeast host recipient distribution are validated and remotely verified. Recipient-level statewide set-asides outside the northeast host allocation and other production fiscal rules remain incomplete.)*
...
- [ ] Fiscal impact. *(The tested engine, Indiana base schedules, venue-specific incumbent/fixed supplemental rates, current admission/device non-applicability, and enacted northeast host distribution are validated; recipient-level statewide set-asides outside the northeast host allocation, other state/local fiscal inputs, and broader jurisdiction coverage remain incomplete.)*
